...
def create_user(name: str, email: str, password: str) -> dict:
    ...
    except sqlite3.IntegrityError as exc: raise ValueError("An account with this email already exists.") from exc

def authenticate(email: str, password: str):
    with sqlite3.connect(DB_PATH) as con:
        row=con.execute("SELECT id,name,email,password_hash FROM users WHERE email=?",(email.strip().lower(),)).fetchone()
...
def get_user(token: str):
    ...
    with sqlite3.connect(DB_PATH) as con:
        row=con.execute("SELECT u.id,u.name,u.email FROM auth_sessions s JOIN users u ON u.id=s.user_id WHERE s.token_hash=? AND s.expires_at>?",(token_hash,time.time())).fetchone()
    return {"id":row[0],"name":row[1],"email":row[2]} if row else None

def revoke_session(token: str) -> None:
    with sqlite3.connect(DB_PATH) as con: con.execute("DELETE FROM auth_sessions WHERE token_hash=?",(hashlib.sha256(token.encode()).hexdigest(),))
