"""Local tools and persistent database-backed memory."""
...
def save_turn(session_id,user_text,assistant_text):
    with connect(DB_PATH) as con:
        con.execute("INSERT INTO messages(session_id,role,content) VALUES(?,?,?)",(session_id,"user",user_text)); con.execute("INSERT INTO messages(session_id,role,content) VALUES(?,?,?)",(session_id,"assistant",assistant_text))
...
def list_sessions(limit=50):
    ...
    with connect(DB_PATH) as con: rows=con.execute("SELECT session_id,MAX(id) AS last_id FROM messages GROUP BY session_id ORDER BY last_id DESC LIMIT ?",(limit,)).fetchall()
    return [session_id for session_id,_ in rows]
def remember_fact(fact,source="user"):
    fact=fact.strip()
    if not fact:return
    with connect(DB_PATH) as con:
        if using_postgres(): con.execute("INSERT INTO memories(fact,source) VALUES(?,?) ON CONFLICT(fact) DO UPDATE SET source=excluded.source",(fact,source))
        else:
...
def semantic_recall_memories(query,limit=8):
    ...
    except Exception:return recall_memories(query,limit=limit)
...
def run_shell(command):
    ...
    try:
        result=subprocess.run(parts,cwd=FILE_ROOT,capture_output=True,text=True,timeout=SHELL_TIMEOUT,shell=False); output=(result.stdout+result.stderr).strip() or "(no output)"
...
def set_chat_title(session_id,title):
    ...
    with connect(DB_PATH) as con: con.execute("INSERT INTO chat_metadata(session_id,title) VALUES(?,?) ON CONFLICT(session_id) DO UPDATE SET title=excluded.title",(session_id,title))
