...
function App(){
 ...
 async function send(){
  ...
  try{
   const r=await fetch(API+"/api/v1/chat/stream",{method:"POST",headers:{"Content-Type":"application/json",Authorization:"Bearer "+token},body:JSON.stringify({message,session_id:"web-"+active,attachment_paths:attachments.map(a=>a.path)}),signal:controller.signal});
