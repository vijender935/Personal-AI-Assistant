...
function App(){
 ...
 return <div className="app">
  ...
  {filesOpen&&<Modal title="Files" onClose={()=>setFilesOpen(false)}><input className="file-search" placeholder="Search files..." value={fileQuery} onChange={e=>setFileQuery(e.target.value)}/>{files.length===0?<p>No uploaded files.</p>:files.filter(f=>f.path.toLowerCase().includes(fileQuery.toLowerCase())).map(f=><div key={f.path} style={{display:"flex",alignItems:"center",gap:8,padding:"9px 0",borderBottom:"1px solid #eee"}}><Files size={16}/><span style={{flex:1,overflow:"hidden",textOverflow:"ellipsis"}}>{f.path}</span><small>{f.rag_indexed?"RAG ✓":"Not indexed"} · {Math.ceil(f.size/1024)} KB</small><button className="icon" onClick={()=>downloadFile(f.path)}><Download size={16}/></button><button className="icon" onClick={()=>deleteFile(f.path)}><Trash2 size={16}/></button></div>)}</Modal>}
  {memoryOpen&&<Modal title="Memory" className="settings-large" onClose={()=>setMemoryOpen(false)}><p>Your saved assistant memories.</p><div className="connector-form"><input placeholder="Remember something..." value={memoryForm} onChange={e=>setMemoryForm(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"){e.preventDefault();saveMemory()}}}/><button className="auth-submit" onClick={saveMemory}>Save memory</button></div><div className="connector-list">{memories.length===0?<p>No saved memories.</p>:memories.map((m,i)=><div className="connector-item" key={i}><div><span>{m}</span></div><button className="chat-more" onClick={()=>deleteMemory(m)}>Delete</button></div>)}</div></Modal>}
  {settings&&<Modal title="Settings" className="settings-large" onClose={()=>setSettings(false)}><p>Signed in as <b>{user?.email}</b></p><p>Model: server-configured Groq model</p><hr/><h3>MCP & Connectors</h3><p className="settings-note">Yahan apna remote MCP server add karo. Abhi HTTP/SSE connectors supported hain.</p><div className="connector-form"><input placeholder="Name (e.g. GitHub)" value={connectorForm.name} onChange={e=>setConnectorForm({...connectorForm,name:e.target.value})}/><select value={connectorForm.transport} onChange={e=>setConnectorForm({...connectorForm,transport:e.target.value})}><option value="streamable-http">Streamable HTTP</option><option value="sse">SSE</option></select><input placeholder="MCP server URL" value={connectorForm.url} onChange={e=>setConnectorForm({...connectorForm,url:e.target.value})}/><input placeholder="Allowed tools (optional, comma separated)" value={connectorForm.allowed_tools} onChange={e=>setConnectorForm({...connectorForm,allowed_tools:e.target.value})}/><button className="auth-submit" onClick={addConnector} disabled={connectorLoading}>{connectorLoading?"Adding...":"Add connector"}</button></div><div className="connector-list">{connectors.length===0?<p>No connectors added.</p>:connectors.map(c=><div className="connector-item" key={c.id}><div><b>{c.name}</b><small>{c.transport} · {c.url}</small></div><button className="chat-more" onClick={()=>deleteConnectorById(c.id)}>Delete</button></div>)}</div></Modal>}
 </div>
}
